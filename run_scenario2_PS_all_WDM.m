...
kur_INT  = kur_COI;                      % All WDM channels are shaped
...
%% Fig. 4(a): SCI over distance
fig = figure('Color','w','Name','Fig. 4(a) - Scenario II SCI');
...
saveas(fig,'results/fig4a_scenario2_SCI.fig');
print(fig,'results/fig4a_scenario2_SCI.png','-dpng','-r300');

%% Fig. 4(b): XCI over distance
fig = figure('Color','w','Name','Fig. 4(b) - Scenario II XCI');
set(fig,'Units','pixels','Position',figure_position);
plot(distance,XCI(1,:),'-', 'Color',blue,   'LineWidth',line_width); hold on;
plot(distance,XCI(2,:),'--','Color',red,    'LineWidth',line_width);
plot(distance,XCI(3,:),':', 'Color',yellow, 'LineWidth',line_width);
plot(distance,XCI(4,:),'-.','Color',purple, 'LineWidth',line_width);
grid on; box on;
xlim([80 800]); ylim([0 5000]);
set(gca,'XTick',[200 400 600 800],'FontName','Times New Roman','FontSize',10);
xlabel('Distance [km]');
ylabel('XCI [W^{-2}]');
title('(b) XCI over distance','FontWeight','bold');
legend('P_1','P_2','P_3','P_4','Location','best');
saveas(fig,'results/fig4b_scenario2_XCI.fig');
print(fig,'results/fig4b_scenario2_XCI.png','-dpng','-r300');

%% Fig. 4(c): MCI over distance
fig = figure('Color','w','Name','Fig. 4(c) - Scenario II MCI');
set(fig,'Units','pixels','Position',figure_position);
...
saveas(fig,'results/fig4c_scenario2_MCI.fig');
print(fig,'results/fig4c_scenario2_MCI.png','-dpng','-r300');

%% Fig. 5(a): NLI over distance
fig = figure('Color','w','Name','Fig. 5(a) - Scenario II NLI');
set(fig,'Units','pixels','Position',figure_position);
...
legend('P_1','P_2','P_3','P_4','Location','best');
saveas(fig,'results/fig5a_scenario2_NLI.fig');
print(fig,'results/fig5a_scenario2_NLI.png','-dpng','-r300');

%% Fig. 5(b): Effective SNR over distance
fig = figure('Color','w','Name','Fig. 5(b) - Scenario II Effective SNR');
set(fig,'Units','pixels','Position',figure_position);
...
legend('P_1','P_2','P_3','P_4','Location','best');
saveas(fig,'results/fig5b_scenario2_SNR_eff.fig');
print(fig,'results/fig5b_scenario2_SNR_eff.png','-dpng','-r300');

%% Fig. 5(c): SER over distance
fig = figure('Color','w','Name','Fig. 5(c) - Scenario II SER');
set(fig,'Units','pixels','Position',figure_position);
...
grid on; box on;
xlim([80 800]); ylim([0 0.20]);
set(gca,'XTick',[200 400 600 800],'FontName','Times New Roman','FontSize',10);
xlabel('Distance [km]');
ylabel('SER');
title('(c) SER over distance','FontWeight','bold');
legend('P_1','P_2','P_3','P_4','Location','best');
saveas(fig,'results/fig5c_scenario2_SER.fig');
print(fig,'results/fig5c_scenario2_SER.png','-dpng','-r300');
...
for i = 1:nD
    ...
    for p = 1:nP
        NLIN = chi11 + (kur_INT(p)-2)*chi21 + chi13 + (kur_INT(p)-2)*chi23 + ...
